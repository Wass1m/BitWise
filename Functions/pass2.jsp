...
<%
          ...
          Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/tablewise","root","toor");
                    ...
                    while(rs.next()){
                        A=Boolean.logicalOr(A,X.equals(rs.getString("email")) );
                   
                        B=Boolean.logicalOr(B,sn.equals(rs.getString("username")) );
                    }
                   if(Boolean.logicalOr(A,B))
                    {   
                        if(Boolean.logicalAnd(A,B)) {bad="Username and Email Taken";}
                        else if(B) {bad="Identifiant existe vous pouvez le changer";}
                        ...
                        out.println("Identifiant n'existe pas ");
                        out.println("</font>");
