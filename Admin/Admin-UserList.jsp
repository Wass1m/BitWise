...
<div id="wrap">
    ...
    if (session.getAttribute("ID") != null) {
        ...
        try {
            ...
            ps.setInt(1, idi);                                 // CONNEXION BASE DE DONNEES
            rsi= ps.executeQuery();
            rsi.first();
...
%>
        ...
        <div class="container">
            <ul class="row">
                <li class="col-sm-8">
                    ...
                    <!--======= BLOG POST=========-->
                    <div class="blog-post"> <img class="img-responsive" src="images/users.jpg" alt="" > <span class="date"><i class="fa fa-clock-o"></i> <%= new Date() %></span> <a class="title-hed">know more about our users</a>
    ...
    <table id="users" class="table table-striped table-bordered table-hover table-inverse" cellspacing="0" width="100%" >
        <thead>
        <tr>
            <th>ID</th>
            <th>USERNAME</th>
        ...
        <%
                                               // RECUPERATION DES INFOS USERS POUR LA LISTE
            ...
            try{
                ...
                ResultSet rs= st.executeQuery("select * from identifiant");
                while(rs.next()){
...
if(type.equals("Client")) {
            ...
            %>
            <td> <button type="button" style="margin-bottom: 20px" class="btn btn-sm btn-default demo2" data-toggle="modal" data-target="#myModal2" onclick="document.getElementById('IDold1').value='<%=rs.getString("ID")%>'">EDIT</button>
                <button type="button" style="margin-left: 80px; margin-bottom: 20px"   class="btn btn-sm btn-default" data-toggle="modal" data-target="#ID01"  onclick="document.getElementById('IDC').value='<%=rs.getString("ID")%>'">Archive</button>
            </td>
<% } else { %>
<td> <button type="button" style=" margin-bottom: 20px" class="btn btn-sm btn-default demo2" data-toggle="modal" data-target="#myModal" onclick="document.getElementById('IDold').value='<%=rs.getString("ID")%>'">EDIT</button>
                <button type="button" style="margin-left: 80px; margin-bottom: 20px"   class="btn btn-sm btn-default" data-toggle="modal" data-target="#ID01"  onclick="document.getElementById('IDC').value='<%=rs.getString("ID")%>'">Archive</button>
            </td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">User Archiving</h4>
      </div>
        
            
      <div class="modal-body">
          <p style="color:red;"><b>DO YOU WANT TO PROCEED ?!<b></p>
        
        <div class="row">
            <div class="col-12-xs text-center">
                <form action='./Functions/Admin-Archivage.jsp' > 
                <input type="hidden" id="IDC" name="IDC" value="" >
                     <button type="submit" value='submit' class="btn btn-success btn-md" style="width:30%; margin-left:-200px;">oui</button> </form>
                     <button class="btn btn-default" data-dismiss="modal" style="width:30%; margin-top:-65px; margin-right:-300px;" >non</button>
            </div>
        </div>
      </div> 
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content" style="width:806px; right:200px; top:auto">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Entrez Les Informations à changer</h4>
            </div>
            <div class="modal-body">
                <form action="./Functions/Admin-EditUser.jsp">
                   
                    
                    <div class="form-group">
                        <input  type="hidden" name="IDold" id="IDold"  value="">
                    </div>
                    <br>
                    <div class="form-group">
                        <input   type="text" name="username" id="username" class="form-control input-sm" placeholder="Nouveau Username"  onkeyup="loadXMLDoc()">
                    </div>
                    <div class="form-group">
                        <input type="email" name="email" id="email" class="form-control input-sm" placeholder="E-mail"  onkeyup="loadXMLDoc()" >
                    </div>
                    <div class="form-group">
                        <input type="password" name="password" id="password" class="form-control input-sm" placeholder="Password" >
                    </div>
                    <div class="form-group">
                       <input type="password" name="password_confirmation" id="password_confirmation" class="form-control input-sm" placeholder="'Password Confirmation" onkeyup="checkPass3(); return false;">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content" style="width:806px; right:200px; top:auto">
            ...
            <div class="modal-body">
                <form action="./Functions/Admin-EditUser.jsp"> 
                     <div class="form-group">
                        <input  type="hidden" name="IDold1" id="IDold1"  value="">
                    </div>
                    <br>
                    <div class="form-group">
                        <input type="password" name="password" id="password1" class="form-control input-sm" placeholder="Password" >
                    </div>
                    <div class="form-group">
                       <input type="password" name="password_confirmation" id="password_confirmation1" class="form-control input-sm" placeholder="'Password Confirmation" onkeyup="checkPass2(); return false;">
